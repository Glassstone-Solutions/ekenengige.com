            ...
            if ( class_exists('Wyde_Importer') && current_user_can( 'manage_options' ) && isset( $_GET['type'] ) && !empty($_GET['type']) ) {
                ...
                try{
                    ...
                    switch( strtolower($type) ){
                        case 'pages':
                        $importer->import_demo_content(get_template_directory() . '/admin/data/'.$demo.'/page.xml'); 
                        break;
                        case 'posts':
                        $importer->import_demo_content(get_template_directory() . '/admin/data/post.xml'); 
                        break;
                        case 'portfolios':
                        $importer->import_demo_content(get_template_directory() . '/admin/data/portfolio.xml'); 
                        break;
                        case 'testimonials':
                        $importer->import_demo_content(get_template_directory() . '/admin/data/testimonial.xml'); 
                        break;
                        case 'team members':
                        $importer->import_demo_content(get_template_directory() . '/admin/data/team-member.xml'); 
                        break;
                        case 'widgets':
                        $importer->import_widgets(get_template_directory_uri() . '/admin/data/'.$demo.'/widget_data.json');
                        break;
                        case 'sliders':
                        $importer->import_revsliders(get_template_directory() . '/admin/data/'.$demo.'/revsliders/');
                        break;
                        case 'settings':
                        $importer->import_demo_content(get_template_directory() . '/admin/data/'.$demo.'/menu.xml'); 
